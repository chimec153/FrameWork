...
bool Bug::Init()
{
	...
	pAni->AddClip("BugIdleLeft", AT_ATLAS, AO_LOOP,
		1.f, 4, 5, 0, 3, 2, 1, 1.f, "bug", TEXT("Characters\\Monsters\\bug.bmp"));
...
int Bug::Update(float fDeltaTime)
{
	...
	if (m_iHP <= 0.f)
	{
		...
		for (int j = 0; j < 2; ++j)
		{
			for (int i = 0; i < 2; ++i)
			{
				Effect* pEffect = (Effect*)CreateCloneObj("BugEffect", "BugEffect", SC_CURRENT, m_pLayer);
