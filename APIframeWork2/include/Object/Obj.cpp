...
Obj::~Obj()
{
	...
	/*if (m_pTexture)
	{
		string strTag = m_pTexture->GetTag();

		if (m_pTexture->Release() == 0)
			GET_SINGLE(ResourcesManager)->EraseTexture(strTag);
	}*/
}
	...
	const wchar_t * pFileName, const string & strPathKey)
{
	...
	m_pAnimation->AddClip(strName, eType, eOption, fAnimationLimitTime,
		...
		fOptionLimitTime, iOptionFrame, strTextKey, pFileName, strPathKey);
	return true;
}
	...
	const vector<wstring>& vecFileName, const string & strPathKey)
{
	...
	m_pAnimation->AddClip(strName, eType, eOption, fAnimationLimitTime,
		iFrameMaxX, iFrameMaxY, iStartX, iStartY, iLengthX, iLengthY,
		fOptionLimitTime, iOptionFrame, strTextKey, vecFileName, strPathKey);
...
{
	SceneCollision* pCollision = m_pScene->GetSceneCollision();
	...
	if (m_pTexture && bInClient)
	{
		...
		if (m_pAnimation)
		{
			...
			if (pClip->eType == AT_ATLAS)
			{
				...
				if (iSize != 0)
				{
					tImagePos.x = pClip->vecFrame[pClip->iFrameX].tStart.x;
					tImagePos.y = pClip->vecFrame[pClip->iFrameX].tStart.y;

					tPos.x += pClip->vecRenderPos[pClip->iFrameX].x;
					tPos.y += pClip->vecRenderPos[pClip->iFrameX].y;
				}
...
{
	Obj*	pProto = m_pScene->FindProtoType(strPtototypeKey);
