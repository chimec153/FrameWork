...
int Fly::Update(float fDeltaTime)
{
	...
	if (m_iHP <= 0.f)
	{
		...
		for (int j = 0; j < 2; ++j)
		{
			for (int i = 0; i < 2; ++i)
			{
				Effect* pEffect = (Effect*)CreateCloneObj("FlyEffect", "FlyEffect", SC_CURRENT, m_pLayer);
